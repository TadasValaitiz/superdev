...
main() {
    ...
    if [[ "$dir" == "$repo/.superdev/sdd" ]]; then
        pass "prints <repo-root>/.superdev/sdd"
    else
        fail "prints <repo-root>/.superdev/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.superdev/sdd/.gitignore" && "$(cat "$repo/.superdev/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.superdev/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.superdev/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.superdev/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.superdev/sdd" && "$wt_dir" != "$dir" ]]; then
