...
EOF
    ...
    cat > "$repo/assets/superdev-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superdev/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/superdev/.fixture-keep"
    cat > "$repo/plugins/superdev/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/superdev/.fixture-keep
    git -C "$repo" add plugins/superdev/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/superdev/skills/example/agents"

    cat > "$repo/plugins/superdev/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/superdev/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superdev/.codex-plugin" \
        "$repo/plugins/superdev/.private-journal" \
        "$repo/plugins/superdev/assets" \
        "$repo/plugins/superdev/hooks" \
        "$repo/plugins/superdev/skills/example/agents" \
        "$repo/plugins/superdev/skills/example"

    cat > "$repo/plugins/superdev/.codex-plugin/plugin.json" <<EOF
{
  "name": "superdev",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/superdev/assets/superdev-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/superdev/assets/app-icon.png"

    cat > "$repo/plugins/superdev/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/superdev/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/superdev/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/superdev/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/superdev/hooks/session-start" "$repo/plugins/superdev/hooks/session-start-codex" "$repo/plugins/superdev/hooks/run-hook.cmd"
...
EOF
    ...
    git -C "$repo" add \
        plugins/superdev/.codex-plugin/plugin.json \
        plugins/superdev/assets/app-icon.png \
        plugins/superdev/assets/superdev-small.svg \
        plugins/superdev/hooks/hooks-codex.json \
        plugins/superdev/hooks/run-hook.cmd \
        plugins/superdev/hooks/session-start \
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superdev/.kimi-plugin" \
        "$repo/plugins/superdev/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superdev/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/superdev/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/superdev/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/superdev/.fixture-keep \
        plugins/superdev/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/superdev/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/superdev/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/superdev-small.svg" "Preview includes SVG asset"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/superdev/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/superdev" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/superdev-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/superdev'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/superdev-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/superdev-*" "Clean no-op local apply does not create sync branch in destination checkout"
