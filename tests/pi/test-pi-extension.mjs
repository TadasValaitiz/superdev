...
const extensionPath = resolve(repoRoot, '.pi/extensions/superdev.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-superdev/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'superdev');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/superdev.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have superdev/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have superdev/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have superdev/);
