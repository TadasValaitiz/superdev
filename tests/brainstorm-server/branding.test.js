/**
 * Tests for the visual companion's Superdev/Prime Radiant branding.
 */
...
function createPackagedServerFixture(version) {
  const root = fs.mkdtempSync(path.join('/tmp', 'superdev-packaged-server-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'superdev', version }, null, 2)
  );
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Superdev v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Superdev v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Superdev v/.test(html),
    'visible logo should appear before the Superdev version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Superdev v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Superdev branding'
  );
}
...
async function main() {
  ...
  await test('packaged Codex plugin reads version from .codex-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes('Superdev vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
  ...
  await test('SUPERDEV_DISABLE_TELEMETRY=true omits remote image but keeps local branding', async () => {
    const port = 3453;
    const dir = '/tmp/brainstorm-branding-disabled';
    await withServer({ port, dir, env: { SUPERDEV_DISABLE_TELEMETRY: 'true' } }, async () => {
      writeFragment(dir);
      await sleep(300);
      const html = await fetchHtml(port);
  ...
  await test('SUPERDEV_DISABLE_TELEMETRY=yes also omits the remote image on the waiting screen', async () => {
    const port = 3454;
    const dir = '/tmp/brainstorm-branding-disabled-waiting';
    await withServer({ port, dir, env: { SUPERDEV_DISABLE_TELEMETRY: 'yes' } }, async () => {
      const html = await fetchHtml(port);
      assertBrandedFallbackText(html);
      assert(!html.includes(ASSET_URL), 'disabled telemetry should omit the remote image');
    });
