...
assert_equal(marketplace.get("name"), "superdev-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "superdev"]
assert_equal(len(matching_plugins), 1, "superdev plugin entry count")
