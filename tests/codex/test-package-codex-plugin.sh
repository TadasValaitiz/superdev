...
archive="$TEST_ROOT/superdev"
tar_archive="$TEST_ROOT/superdev.tar.gz"
...
unexpected_pattern='(^superdev/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)'
...
archive_from_tar_source="$TEST_ROOT/superdev-from-tar-source.zip"
archive_from_zip_source="$TEST_ROOT/superdev-from-zip-source.zip"
(
  cd "$metadata_source"
