...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERDEV_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPERDEV_SKILLS_DIR/using-superdev/SKILL.md" "$TEST_HOME/using-superdev.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERDEV_PLUGIN_FILE" missing
}
