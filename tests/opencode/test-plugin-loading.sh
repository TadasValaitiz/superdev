#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the superdev plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/superdev.js"
...
skill_count=$(find "$SUPERDEV_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPERDEV_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-superdev skill exists (critical for bootstrap)
echo "Test 3: Checking using-superdev skill (required for bootstrap)..."
if [ -f "$SUPERDEV_SKILLS_DIR/using-superdev/SKILL.md" ]; then
    echo "  [PASS] using-superdev skill exists"
else
    echo "  [FAIL] using-superdev skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPERDEV_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/superdev/' "$SUPERDEV_PLUGIN_FILE"; then
