...
# 1. Create in superdev location (lowest priority)
mkdir -p "$SUPERDEV_SKILLS_DIR/priority-test"
cat > "$SUPERDEV_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superdev version of priority test skill
---
# Priority Test Skill (Superdev Version)

This is the SUPERDEV version of the priority test skill.

PRIORITY_MARKER_SUPERDEV_VERSION
EOF
...
if [ -f "$SUPERDEV_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superdev version exists"
else
    echo "  [FAIL] Superdev version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled superdev priority
echo ""
echo "Test 2: Documenting personal vs superdev priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superdev version instead of personal version"

# Test 3: Document project vs bundled superdev priority
echo ""
echo "Test 3: Documenting project vs personal/superdev priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superdev version instead of project version"

# Test 4: Test a non-colliding bundled superdev skill is still available
echo ""
echo "Test 4: Testing non-colliding superdev skill remains available..."

mkdir -p "$SUPERDEV_SKILLS_DIR/superdev-only-test"
cat > "$SUPERDEV_SKILLS_DIR/superdev-only-test/SKILL.md" <<'EOF'
---
name: superdev-only-test
description: Superdev-only priority test skill
---
# Superdev Only Test Skill

PRIORITY_MARKER_SUPERDEV_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"superdev-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERDEV_ONLY_VERSION" "Non-colliding superdev skill is still registered"
