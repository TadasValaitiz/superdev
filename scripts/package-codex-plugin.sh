#!/usr/bin/env bash
#
# Package the Superdev Codex plugin as a rootless archive for portal upload.
#
...
Options:
  --output PATH            Write archive to PATH.
                           Default: ../_tmp/sup-codex-packaging/superdev-VERSION.zip
  ...
  --metadata-source PATH   Prior official package directory, .zip, or .tar.gz used to
                           ...
                           Default: ../_tmp/sup-codex-packaging/superdev,
                           falling back to superdev.zip, then superdev.tar.gz
...
if [[ -z "$METADATA_SOURCE" ]]; then
  if [[ -d "$REPO_ROOT/../_tmp/sup-codex-packaging/superdev" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superdev"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/superdev.zip" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superdev.zip"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/superdev.tar.gz" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superdev.tar.gz"
  else
    die "no metadata source found; pass --metadata-source <prior package dir, zip, or tar.gz>"
  fi
fi

WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/superdev-codex-package.XXXXXX")"
...
if [[ -z "$OUTPUT" ]]; then
  case "$FORMAT" in
    zip)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/superdev-$VERSION.zip"
      ;;
    tar.gz)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/superdev-$VERSION.tar.gz"
      ;;
  esac
fi
...
unexpected_paths="$(
  printf '%s\n' "$archive_paths" |
    grep -E '(^superdev/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)' || true
)"
