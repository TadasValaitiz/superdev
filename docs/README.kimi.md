# Superdev for Kimi Code

Complete guide for using Superdev with [Kimi Code](https://github.com/MoonshotAI/kimi-code).
...
2. Loads `using-superdev` at session start through `sessionStart.skill`.
3. Provides Kimi-specific tool mapping through `skillInstructions`.

Kimi Code reads Superdev skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Superdev and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info superdev` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Superdev release, install the branch explicitly:
...
1. Confirm `/plugins info superdev` shows the plugin enabled.
