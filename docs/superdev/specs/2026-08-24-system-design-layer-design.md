...
**Mode:** human-in-loop
**Decision log:** ./2026-08-24-system-design-layer-decisions.md (D1–D38; the arbitration surface is the addendum **plus each entry's own revisit-when**)
...
**Origin:** brainstorm with Tadas · **Angle companions:** `./2026-08-24-system-design-layer-angle-01-item-journey.md` · `…-angle-02-authority-ledger-algebra.md` · `…-angle-03-operator-attention.md`
...
D1 corpus is law + persistent human-driven architect · D2 ledgers truth/messages pointers · D3 no publish gate; markers + forward conformance · D4 front desk required · D5 altitude split future/present · D6 update existing plugins · D7 item room owns worktree+merge · D8 orchestrator collects, architect works per-checkpoint · D9 ad-hoc rooms allowed · D10 item internals: wider tasks, role split (provisional → deep-dive) · D11 milestone = implementation boundary; closes with handoff · D12 vision docs · D13 backlog not docket · D14 MIG-MARK/DOC-MARK grammars · D15 glossary closed (handoff·bridge·checkpoint) · D16 test clearance (provisional → deep-dive) · D17 corpus layout canonical+extensible · D18 architect idles; design human-driven; orchestrator owns design-dry pause · D19 checkpoint = rule+green lights+feel · D20 handover protocol two-way with agree/disagree · D21 scoped pause · D22 no micro tier · D23 angle contract + anti-loosening · D24 §5.4 lands in superdev's orchestrator (ratified, D33) · D25 process-improvement two-speed loop · D26 item angles, corpus cited at line level · D27 operational strategy HIL→protocol · D28 item-angle location, execution-shape proposals, archive lifecycle · D29 operator arc: agree shape → auto-start · D30 operator-absent charter waits at ratification (ratified, D33) · D31 snapshots (superseded by D32) · D32 resume over snapshot, Codex first-class not default, long review cadence · D33 ratification by delegation (D24, D30, naming pairs) · D34 plan forks · D35 shape A · D36 long arcs; Codex = long-run + resume-first · D37 one deep initial write; parallel = reads + quick-fix phases only · D38 red-green by role — implementer inner loop, reviewer test-adversary with watched-to-fail duty; archive sweep = first checkpoint; **R22 deep-dive complete, D10/D16 ratified**. Revisit-when hooks for the load-bearing rulings live in the log's 2026-08-25 addendum; entries recorded before it carry rulings without weighed alternatives — the addendum, not this section, is the arbitration surface.

## 7. Assumptions & open questions

| ID | Assumption / question | Affects | Status |
|----|----------------------|---------|--------|
| A1 | The role-split item-room internals (one implementer carrying wider tasks + reviewer + follow-up) outperform bite-size task fan-out with current Codex/Sonnet workers | R22, 5.5 | ratified by the R22 deep-dive (D36–D38); empirical paired trial still worth running at rollout |
...
| A6 | Harvest→delete→rewrite is the right default test clearance for REPLACE'd areas | R20, 5.5 | ratified (D38: sweep = first checkpoint, reviewer signs the harvest, cleanup at close gate) |
| A7 | D24: superdev's orchestrator is the right home for §5.4 (room-graph's stays generic) | 5.9 | ratified (D33) |
| A8 | D30: self-brainstorm-to-ratification is the right operator-absent behaviour | R4, 5.5 | ratified (D33) |
...
|---|----------------------------------|--------|------|---------|
| AH1 | A solo `/system-design` run in a fresh project produces the canonical corpus with at least one angle, a map, and a D# log — and a second session grounds on it without re-deriving | UC1, R1, R15 | slow | GREEN run 2026-08-25: fresh `/tmp/green-test/fresh` → all 9 corpus entries, contract-honouring angle, 2-row map, D1; second session grounded via BLIND grep + decisions.md without re-deriving. Transcript: `docs/superdev/reviews/` + task af49/ae3f outputs. Skill: `skills/system-design/` @ superdev 7.4.0. |
...
| AH8 | An item brainstorm states its test disposition explicitly; a harvest-delete-rewrite item shows harvested requirements before deletion | R20 | slow | Skill-text half shipped 7.5.0: brainstorming 6b disposition question + template §7b; harvest reviewer-signed BEFORE archiving (test-clearance.md); GREEN abbd5 PART 2 walks the full lifecycle. Live half at rollout. |
| AH9 | Grepping all edited skills finds the shared glossary, no retired term used in its old sense, and the residue/residual pair used only in their distinguished senses | UC10, R16 | fast | Greps 2026-08-25: `## Milestone M|Milestone gate` in writing-plans → 0 (was 14); docket/runway → only the glossary's own retirement notices; residue/residual + GREEN-LIGHT rows verified in room-communication compliance. SDD parallel-execution keeps old vocabulary, DOC-MARK[DEFERRED][wave-2] planted (wave-2 receipt). |
| AH10 | Every skill edit follows writing-skills discipline: a baseline failure observed before the edit, compliance after | 5.9, R23 | slow | Every wave-1 edit ran baseline→edit→compliance: system-design (RED af497/GREEN ae3f6) · orchestrator (a7d9b/a84e9) · self-improvement+brainstorming (a664c/a84be) · four small skills (acd98/aa82f JOB1) · room-graph (aa82f JOB2/a1e86). Baseline record: `docs/superdev/reviews/2026-08-25-system-design-baseline.md`. |
...
| AH16 | An item brainstorm on corpus-governed territory opens with system passages quoted at file:line, runs angle-by-angle, and ships a spec with item angle companions and per-area statuses | R25 | slow | Simulated (compliance a84be PART 2): corpus quoted at file:line at step 1, vision grounding for RESHAPE, item angle beside the spec per template, §5 DOC-MARK status line, LOCKED-collision → residue. Live receipt lands at rollout. |
| AH17 | A plan for model-changing work presents execution-shape variants before any task exists; its swept tests sit in `tests/_archived/` with manifest + harvest, one is recalled mid-build, and post-development cleanup leaves only the manifest | R26 | slow | Skill-text half shipped 7.5.0: writing-plans proposes 2–3 shape variants, agreement = auto-start; archive lifecycle in test-clearance.md; GREEN abbd5 quotes verbatim. Live half at rollout. |
...
| AH19 | A reviewer finding is fixed by resuming the original implementer (SendMessage or `codex-worker run --name`) with only the findings; the registry shows resume metadata for every subagent of the arc; the broad arc was reviewed at checkpoints, not per-mini-task | R28 | slow | Skill-text half shipped 7.5.0: resume-first fixes, RESUME block mandatory, checkpoint-cadence reviews (SDD arc model); GREEN abbd5. Live half at rollout. |
