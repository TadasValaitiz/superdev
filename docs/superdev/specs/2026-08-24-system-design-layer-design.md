...
| R28 | **Reground snapshots (D31):** implementers write a tracked snapshot (territory map · invariants/traps · fast verify commands · unfinished edges) at task end and every plan checkpoint; a registry maps territory→snapshot; dispatches into overlapping territory carry the relevant snapshots; fixes go by SendMessage continuation to the still-addressable implementer first, fresh dispatch + snapshot second | D31 | must | a fix or neighbouring task starts from a pasted snapshot, not a cold re-derivation |
...
| AH19 | A reviewer finding is fixed by messaging the original implementer while addressable; later, a task entering the same territory starts with the pasted snapshot and its fast verify commands rerun | R28 | slow | |
