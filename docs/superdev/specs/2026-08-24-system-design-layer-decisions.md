...
## D44 — Dependencies are never parallelised: producer merges first; parallelism is created by SPLITTING blockers
**When:** 2026-08-26T04:54:11Z · **Phase:** brainstorm · **Status:** locked · **Decided by:** Tadas
- **Trigger:** cross-room parallelism with separate worktrees needs upfront dependency resolution; a real milestone had one giant item blocking everybody — it could have been ~6 parts, with everyone unblocked after 2 merged.
- **Decided:** (b) **producer-first, serial**: if item B depends on item A, B waits until A has MERGED — no contract-freezing, no stubs, no parallel-across-a-dependency (rejected: contract-first (a) — frozen contracts are fiction where rediscovery happens, and the collision history proves upfront paperwork gets bypassed; stubs (c) — same risk plus throwaway work). Parallelism exists ONLY between fully independent items. Therefore the orchestrator creates parallelism by **splitting blockers**, not by coordinating overlap: a high-blocking item is cut so its **unblocking kernel** — the minimal sub-item producing what dependents consume — merges as early as possible, releasing the rest of the graph while the remainder continues.
- **The best-path algorithm (orchestrator duty):** build the DAG from map rows + bridges · compute each item's blocking radius (transitive dependents) · split any item whose radius or critical-path position warrants it, kernel first · schedule greedily: ready set = all deps merged; launch up to the room budget (capped by operator attention, angle-03); on every merge, recompute and launch the newly ready. Priority = what unblocks the most work soonest, not what is biggest or oldest.
- **Revisit-when:** kernel-splitting produces sub-items too small to justify rooms twice → kernels may be delivered as the FIRST plan checkpoint of the giant item with an early FF-CAS partial publish instead of a separate room.

## D45 — Splits: orchestrator proposes along corpus seams, operator ratifies; every part deliverable+testable; the architect stays holistic
