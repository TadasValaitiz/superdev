...
| R3 | Each common-worker turn that becomes `completed`, `failed`, or `interrupted` emits one versioned terminal callback containing its complete projected result; observation timeouts do not. | stated | must | Claude receives the terminal report for success and non-success without polling. |
...
| R6 | The daemon alone holds callback credentials; public JSON, logs, prompts, and Codex execution never expose the socket token or peer-token store. | stated + measured security boundary | must | Secret scans and public-response checks find no callback credential or raw target path. |
...
| R8 | Callback binding and terminal attempt identity survive daemon restart while the original Claude room remains live; written/uncertain events are not blindly replayed. | inferred from existing durable-resume goal | must | Restart resumes a pending attempt and does not intentionally duplicate a recorded write. |
| R9 | Default and override endpoints are resolved and validated with owner/type/mode/ancestor checks; duplicate live Claude names are refused. | measured protocol + existing socket safety | must | Adversarial endpoints and ambiguous names produce typed refusals before any write. |
| R10 | The callback line respects Claude's measured 1,048,576-character cap, and every public client command emits exactly one JSON object with local usage errors exiting 2. | existing CLI contract + measured protocol | must | Boundary-size and CLI process checks pass. |
...
| UC6 | As an operator, I restart the daemon and retain the originating callback route and honest last-attempt state without a blind duplicate. | R2, R8 | 5.2, 5.4 |
...
and both callback paths pass through one hardened sender.

The flow begins with capture (§5.2), proceeds through validated transport and optional
...
This boundary preserves the current worker architecture: callbacks are additive to the
...
The domain model gives capture, events, attempts, and command models one vocabulary so
the persistence, terminal, proactive, and CLI paths cannot silently disagree (serves
R1–R10; governed by D1–D11; realizes UC1–UC7).
...
classDiagram
  ...
  class CallbackBinding {
    ...
    session_id: UUID
    state: CallbackState
    target_socket: AbsolutePath (secret)
    child_token: HexToken (secret)
...
- Tokens and target sockets are secret persistence fields, never public annotations.
...
| Worker identity | name / session ID / thread ID | CLI / daemon registry / Codex | D9 — proactive routing requires worker `--name`; the event includes all identities. |
...
|---|---|---|---|---|
| ADD | `CallbackBinding` + owner-only `CallbackStore` | no callback persistence | per-session enabled/disabled/unavailable binding separate from public records | D1, D2, D6 |
| ADD | `CallbackEvent` v1 | no callback event contract | terminal and proactive payloads share identity/schema/priority | D2, D7, D10 |
| ADD | `CallbackAttempt` | no send state | pending/written/failed/uncertain with stable event ID | D7, D11 |
| ADD | `StartWorkerRequest.no_callback` and secret `callback_capture` | start ignores Claude messaging env | explicit suppression plus ambient capture over owner-only RPC | D8 |
...
| ADD | `WorkerStatusResponse.callback` | callback invisible | redacted state and last terminal attempt | D7 |
...
| Written/uncertain attempts are not automatically replayed; only pending may resume. | callback-store transition validator and restart tests. |
| The serialized user frame stays within the measured Claude line cap. | transport validates encoded user-line character length before connect. |
...
| `codex-worker message` | `MessageWorkerRequest` | worker name, message, priority, optional `cc_agent_name` | `CallbackSendResponse` |
...
D8; realizes UC1, UC3, UC4, UC6).

- **Design:** The client process validates the presence and basic shape of
  `CLAUDE_CODE_MESSAGING_SOCKET` and `CLAUDE_CODE_MESSAGING_TOKEN`, packages them in an
  internal `CallbackCapture`, and sends it only over the owner-only managed RPC socket.
  `--no-callback` suppresses capture. Missing ambient variables create an unavailable
  record. After the broker creates the logical session and before the first turn starts,
  the façade atomically writes the binding to `callbacks.json` using the same owner,
  mode, fsync, replace, and parent-fsync discipline as the session registry. A callback
  persistence failure returns a typed storage fault with worker/thread recovery IDs and
  does not start the first turn. `run` reads but never updates the binding.
...
- **Serves:** R1, R2, R6, R8 · **Governed by:** D1, D6, D8 · **Realizes:** UC1, UC3,
  UC4, UC6.
...
D2, D5, D6, D10, D11; realizes UC2, UC3, UC7).
...
- **Design:** A stdlib-only transport validates absolute socket paths with `lstat`, owner,
  ...
  default path authenticates with the captured child token. A `cc_agent_name` override
  scans the Claude registry only inside the daemon, selects exactly one live name, derives
  its peer-token filename with `abspath` rather than `realpath`, validates both registry
  and key files, and refuses zero or multiple matches. The sender writes auth then user
  NDJSON lines, includes `from_mode: bypass`, uses the selected priority, sends no
  `session_id`, and half-closes. The encoded user line is rejected before connect if it
  exceeds the measured 1,048,576-character cap.
- **Interface / contract:** transport returns pending/written/failed/uncertain evidence;
  it never returns delivered. No fallback target exists.
- **Depends on:** measured Claude 2.1.237 protocol, callback store, safe-path utilities.
- **Serves:** R5, R6, R9, R10 · **Governed by:** D2, D5, D6, D10, D11 · **Realizes:**
  UC2, UC3, UC7.
...
- **Design:** The hook sits after `_start_and_wait` has produced a terminal
  ...
  event ID is recorded pending before connection. A fully written frame records written;
  an early failure records failed; ambiguous partial/crash cases record uncertain. On
  daemon restart only a persisted pending attempt may resume. The callback operation is
  bounded and its outcome cannot replace or mutate the completion result.
- **Interface / contract:** event schema is `codex-worker.claude-callback/v1`, event
  `turn_terminal`, with worker identity and the completion fields as payload.
- **Depends on:** completion projection, callback store, transport.
- **Serves:** R3, R7, R8 · **Governed by:** D1, D2, D7, D10, D11 · **Realizes:** UC1,
  UC5, UC6.
...
- **Design:** `WorkerFacade.message` resolves the named worker and its stored binding but
  ...
  unique name, the exact `message` syntax, the broadcast-like continue-working rule, and
  the fact that Claude may later steer or run.
...
- **Serves:** R4, R5, R6 · **Governed by:** D4, D5, D9, D10, D11 · **Realizes:** UC2,
  UC3.
...
- **Design:** `worker/message` is a common RPC method and `message` is a top-level common
  ...
  `--cc-agent-name`. It does not autostart a stopped daemon. `start` gains only
  ...
  façade fault vocabulary cover unavailable, target-not-found, ambiguous-target,
  too-large, and send-failed cases. Every local usage refusal occurs before lifecycle
  work, exits 2, and emits one JSON object.
...
security (serves R6, R7, R8, R9, R11; governed by D2, D7, D11; realizes UC4–UC6).
...
- **Design:** Automatic send failure is persisted and projected through status but never
  ...
  Terminal attempt storage is bounded to the latest turn per worker; proactive attempts
  are returned to the caller and need no durable history. Transport logs carry event ID,
  worker name, outcome, and redacted reason only.
...
- **Serves:** R6, R7, R8, R9, R11 · **Governed by:** D2, D7, D11 · **Realizes:** UC4,
  UC5, UC6.

### 5.8 Probe and checkride evidence

The evidence lane separates transport discovery from product acceptance while proving
both against a real responsive Claude room (serves R9–R12; governed by D11, D12, D14;
...
- **Design:** The trading-platform source repository adds two stdlib-only probes that
  ...
  failed/interrupted/no-callback/override journeys are exercised. Existing native Claude
  and raw compatibility are rerun.
...
- **Serves:** R9, R10, R11, R12 · **Governed by:** D11, D12, D14 · **Realizes:** UC7 and
  validates UC1–UC6.
...
### D3 — Allow explicit return-room override (status: locked)
- **Decision:** retain a deliberate override mechanism.
...
| A3 | The child token remains valid for the lifetime of its originating Claude inbox; a dead/restarted Claude process makes the stored route unavailable rather than transferable. | R2, R8, §5.2–5.4 | ratified by measured protocol token lifecycle; stale-target behavior remains testable |
...
- Automatic retry of written or uncertain events — avoids blind duplicate prompts.
...
|---|----------------------------------|--------|------|--------------------------|
| AH1 | Claude starts a named worker with no callback boilerplate and receives the complete successful report without polling. | UC1 / R1–R3 | live checkride | |
...
| AH7 | A daemon restart retains a pending callback and does not intentionally replay a recorded written or uncertain event. | UC6 / R8 | fast process scenario | |
| AH8 | Callback transport rejects unsafe, stale, oversized, missing, and ambiguous targets without leaking secrets or changing turn truth. | UC3–UC6 / R6, R7, R9, R10 | fast + refusal checkride | |
