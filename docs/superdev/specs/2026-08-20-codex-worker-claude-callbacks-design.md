...
| AH5 | Completed, failed, and interrupted turns notify; a wait timeout does not masquerade as completion. | UC5 / R3, R7 | fast + live | **MEASURED:** focused clean recovery `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/focused-reride/callback-recovery-clean/transcript.jsonl` preserves completed, failed, and interrupted frames plus timeout-active then later terminal. |
