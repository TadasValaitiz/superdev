...
|---|----------------------------------|--------|------|--------------------------|
| AH1 | Claude starts a named worker with no callback boilerplate and receives the complete successful report without polling, inline or through a verified artifact reference. | UC1 / R1–R3 | live checkride | **MEASURED:** tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/reviewer-fix-wave/claude-caller-exact/{claude.stream.jsonl,validated-common-evidence.json}`; two ordered receiver attestations each preserve a parseable completion object exactly equal to the corresponding full command result, with automatic IDs, PATH-only commands, and no MCP/raw Codex. Synthetic live corroboration: tracked `callback-common/transcript.jsonl` in the main evidence directory. |
| AH2 | A follow-up launched from different ambient Claude metadata still reports to the worker's original room. | UC1, UC6 / R2, R8 | fast + live | **MEASURED:** tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/callback-origin-retention/transcript.jsonl`; replacement then unset ambient metadata, three origin-only terminal IDs. |
| AH3 | Codex proactively tells Claude something during an active turn, continues working, and accepts a later steer/run response. | UC2 / R4, R6 | live checkride | **MEASURED:** tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/callback-proactive/transcript.jsonl`; written proactive ID, accepted steer, later run terminal. Exactly-five concurrency: tracked `callback-five-workers/transcript.jsonl` beside that directory. |
| AH4 | One proactive message reaches an explicitly named alternate Claude agent while the eventual terminal callback still reaches the origin. | UC3 / R2, R5, R9 | live checkride | **MEASURED:** tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/callback-proactive/transcript.jsonl`; one alternate frame, origin terminal/default follow-up frames. |
| AH5 | Completed, failed, and interrupted turns notify; a wait timeout does not masquerade as completion. | UC5 / R3, R7 | fast + live | **MEASURED:** `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/reviewer-timeout-negative/callback-recovery/transcript.jsonl` records `timeout_active`, then an explicit origin-inbox snapshot with zero matching terminal events for that turn, then exactly one later completed callback with the same turn ID and exact result. The focused clean recovery record preserves failed and interrupted frames. |
| AH6 | Standalone and explicitly callback-disabled workers retain the ordinary worker experience with honest callback state. | UC4 / R1, R11 | fast + live | **MEASURED:** tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/callback-common/transcript.jsonl` records enabled/disabled/unavailable full completions and redacted states. |
| AH7 | A daemon restart retains the complete non-written callback, retries it with the same event ID, exposes the possible crash-window duplicate, and never intentionally replays a recorded `written` event. | UC6 / R8 | fast process scenario | **MEASURED:** focused clean recovery record above: same pending ID retries to written with attempt count 4; already-written ID is not replayed; 801482-byte artifact SHA-256/read-back verifies. Deterministic guards: `CallbackStoreTests.test_failed_attempt_keeps_complete_pending_event_and_written_never_replays`, `CallbackDispatcherTests.test_recovery_retries_same_id_increases_attempt_and_written_is_not_replayed`. |
| AH8 | Callback transport scrubs product-provided Codex child credentials and rejects unsafe, PID-reused, identity-drifted, Unicode-oversized, missing, and ambiguous targets without product-propagating secrets or changing turn truth. | UC3–UC6 / R6, R7, R9, R10 | fast + refusal checkride | **MEASURED:** `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/focused-reride/callback-security/transcript.jsonl`; SCRUBBED child, stale/PID-reuse, ambiguous, daemon-owned Unicode `-32037` with worker IDs/shorter-message recovery, disabled refusal. Full unsafe/not-found matrix: `ClaudeTransportTests`. |
| AH9 | Both stdlib probes send versioned events to `orchestrator-original`, whose append-only pongs correlate by event ID. | UC7 / R12 | live probe | Trading worktree range `581999f0..3302175a` on `codex-worker-callback-probes` includes implementation `4ce3fd0a` and side-effect correction `3302175a`; **MEASURED** pongs seq 3/4 are preserved in tracked `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/trading-probe-pongs.jsonl` (source: trading main `.superdev/brainstorm/codex-worker-claude-callback-ping-pong.jsonl`). |
| AH10 | Existing native Claude, raw worker methods, model/access policy, recovery, and non-destructive stop still work without MCP or cloud messaging. | UC1–UC6 / R11 | fast + live compatibility | **MEASURED:** real Claude receipt above; focused recovery drives raw `session show`/`turn status`; focused stopped-daemon ride proves preserved stop and explicit no-turn start; warning-strict 363-test gate. |
| AH11 | Every callback client outcome is exactly one JSON object and distinguishes written from delivered. | UC2, UC3, UC7 / R7, R10 | fast + live | `CliTests.test_every_client_command_emits_one_json_object`; **MEASURED:** focused reride preserves 21 sanitized destination-labelled frames across original+clean records; client results say `written` only and never claim delivery. Independent evaluator PASS: `docs/superdev/checkrides/2026-08-20-codex-worker-claude-callbacks-evidence/evaluator-verdict.md`. |
