...
1. **Every mutable file has one writer** (or disjoint ID blocks in an append-only log). For **cross-room ledgers** the §5.10 table is exhaustive — an unlisted cross-room write is a violation, not an innovation; room-internal files (the item's own spec, angles, plan, SDD machinery) are governed by the room's charter instead.        LOCKED
...
## Visible collisions
- **Two orchestrator skills** claim the milestone altitude (superdev's vs room-graph's) — reconciled by D24, ratified by D33 (superdev's wins). MISMATCH in skill texts until wave 1.
