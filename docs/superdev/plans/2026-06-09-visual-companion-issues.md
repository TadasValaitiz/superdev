...
- **Remote serving is a first-class scenario.** Superdev is general-purpose;
...
- Env var (e.g. `SUPERDEV_VISUAL_COMPANION=off`) the skill is told to read —
  simplest, matches what the issue asks for, lives in `.zshrc`.
- A plugin-settings file (`.claude/superdev.local.md` frontmatter) — more
...
Today `--project-dir` writes session state to `<project>/.superdev/brainstorm/`
(`start-server.sh:80-84`) and the skill tells the user to gitignore it
(`visual-companion.md:58`). The ask is a `--state-dir` / `SUPERDEV_STATE_DIR`
