...
**Spec:** `docs/superdev/specs/2026-06-09-sdd-task-scoped-review-dispatch-design.md` — read it before starting. Decisions already settled there: full re-reviews stay; the two review stages stay separate; coordinator keeps model judgment; `requesting-code-review/` stays broad.
...
```
- **superdev:requesting-code-review** - Code review template for reviewer subagents
```

with:

```
- **superdev:requesting-code-review** - Code review template for the final whole-branch review
```
...
Lives in the `evals/` **submodule** (separate repo, `superdev-evals`). Work on a branch there; the parent submodule-pointer bump happens at finishing time per `evals/CLAUDE.md`.
...
def scaffold_sdd_quality_defect_plan(workdir: Path) -> None:
    ...
    plans_dir = workdir / "docs" / "superdev" / "plans"
...
You have a small plan at docs/superdev/plans/report-plan.md — two report
...
"I have a small plan at docs/superdev/plans/report-plan.md — two report
formatting functions. Use the superdev:subagent-driven-development skill
...
- A `Skill` invocation naming `superdev:subagent-driven-development`
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists 'docs/superdev/plans/report-plan.md'
    file-contains 'docs/superdev/plans/report-plan.md' 'formatAdminReport'
    file-contains 'docs/superdev/plans/report-plan.md' 'repeat\(40\)'
}

post() {
    skill-called superdev:subagent-driven-development
    tool-called Agent
...
Run: `grep -n "code-quality-reviewer" skills/using-superdev/references/antigravity-tools.md skills/using-superdev/references/gemini-tools.md`
...
- [ ] **Step 1: Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/superdev/superdev`, on dev), or any checkout without this branch's changes:

```bash
cd evals
export SUPERDEV_ROOT=/Users/jesse/git/superdev/superdev
...
- [ ] **Step 2: After (this branch's skills)** — point `SUPERDEV_ROOT` at this worktree:

```bash
cd evals
export SUPERDEV_ROOT=/Users/jesse/git/superdev/superdev/.claude/worktrees/sdd-review-dispatch
uv run quorum run scenarios/sdd-rejects-extra-features --coding-agent claude
uv run quorum run scenarios/sdd-go-fractals --coding-agent claude
uv run quorum run scenarios/sdd-svelte-todo --coding-agent claude
uv run quorum run scenarios/spec-reviewer-catches-planted-flaws --coding-agent claude
...
After all tasks pass: the evals submodule commit needs to land in `superdev-evals` (PR to its `main`), then this branch bumps the `evals` submodule pointer — per `evals/CLAUDE.md`, the parent bump is part of propagation, not optional. Then use superdev:finishing-a-development-branch. PRs against superdev target `dev`.
