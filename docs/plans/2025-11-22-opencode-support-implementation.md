...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superdev:executing-plans to implement this plan task-by-task.

**Goal:** Add full superdev support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superdev-codex` (lines 40-74)
...
- Reference: `.codex/superdev-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superdev' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superdev-codex` (lines 212-280)
 ...
 * (personal skills override superdev skills).
 *
 * @param {string} skillName - Name like "superdev:brainstorming" or "my-skill"
 * @param {string} superdevDir - Path to superdev skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superdevDir, personalDir) {
    // Strip superdev: prefix if present
    const forceSuperdev = skillName.startsWith('superdev:');
    const actualSkillName = forceSuperdev ? skillName.replace(/^superdev:/, '') : skillName;

    // Try personal skills first (unless explicitly superdev:)
    if (!forceSuperdev && personalDir) {
    ...
    // Try superdev skills
    if (superdevDir) {
        const superdevPath = path.join(superdevDir, actualSkillName);
        const superdevSkillFile = path.join(superdevPath, 'SKILL.md');
        if (fs.existsSync(superdevSkillFile)) {
            return {
                skillFile: superdevSkillFile,
                sourceType: 'superdev',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superdev-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superdev-codex` (add import at top)
...
Run: `node -c .codex/superdev-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superdev-codex
...
**Files:**
- Modify: `.codex/superdev-codex` (lines 40-74)
...
Run: `.codex/superdev-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superdev-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superdev-codex` (lines 16-38, approximately)
...
Run: `.codex/superdev-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superdev.js`
...
/**
 * Superdev plugin for OpenCode.ai
 *
...
const superdevSkillsDir = path.join(homeDir, '.config/opencode/superdev/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperdevPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superdev.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superdev.js
...
**Files:**
- Modify: `.opencode/plugin/superdev.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superdev:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superdev)
...
Run: `node -c .opencode/plugin/superdev.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superdev and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superdevSkills = skillsCore.findSkillsInDir(
            superdevSkillsDir,
            'superdev',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superdevSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superdev skills to ~/.config/opencode/superdev/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superdev:';
...
```javascript
    'session.started': async () => {
      // Read using-superdev skill content
      const usingSuperdevPath = skillsCore.resolveSkillPath(
        'using-superdev',
        superdevSkillsDir,
        personalSkillsDir
      );

      let usingSuperdevContent = '';
      if (usingSuperdevPath) {
        const fullContent = fs.readFileSync(usingSuperdevPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperdevContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superdev skills: \`superdev:skill-name\` (from ~/.config/opencode/superdev/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superdev')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superdev && git pull` to update superdev.' :
        '';
...
**Below is the full content of your 'superdev:using-superdev' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperdevContent}
...
```markdown
# Installing Superdev for OpenCode
...
### 1. Install Superdev Skills

```bash
# Clone superdev skills to OpenCode config directory
mkdir -p ~/.config/opencode/superdev
git clone https://github.com/obra/superpowers.git ~/.config/opencode/superdev
```

### 2. Install the Plugin

The plugin is included in the superdev repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superdev/.opencode/plugin/superdev.js`
...
ln -s ~/.config/opencode/superdev/.opencode/plugin/superdev.js .opencode/plugin/superdev.js
```
...
```
use use_skill tool with skill_name: "superdev:brainstorming"
```
...
Personal skills override superdev skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superdev
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superdev/.opencode/plugin/superdev.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superdev/skills`
...
Superdev works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/superdev-codex use-skill superdev:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superdev-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superdev.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superdev.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superdev-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superdev-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superdev.js` created with tools and hooks
