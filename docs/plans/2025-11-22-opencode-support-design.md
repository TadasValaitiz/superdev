...
Add full superdev support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port superdev to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/superdev-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/superdev.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/superdev/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/superdev-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "superdev:brainstorming")')
  }),
...
1. **Inject using-superdev content**
   - Full content of the using-superdev skill
...
```javascript
// .opencode/plugin/superdev.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SuperdevPlugin = async ({ client, directory, $ }) => {
  const superdevDir = path.join(process.env.HOME, '.config/opencode/superdev');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSuperdev = await readSkill('using-superdev');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSuperdev}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── superdev-codex        # UPDATED: Use skills-core
│   ├── superdev-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── superdev.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/superdev-codex`
...
2. Update `.codex/superdev-codex` to use shared core
...
1. Create `.opencode/plugin/superdev.js`
