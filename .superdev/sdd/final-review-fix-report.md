...
## Follow-up final-review correction — complete managed startup normalization

**Finding:** The first fix normalized setup exceptions but still rethrew several
prebuilt `-32024` faults unchanged. Unsafe/changed/accepting sockets, spawn failure,
child early exit, and readiness timeout therefore exposed different subsets of the
managed recovery contract.

**Root cause:** `ensure_running()` treated `FacadeFault` as already public-ready while
individual lifecycle branches independently constructed incomplete faults.

**Fix:** `InstanceManager._start_fault()` is now the single managed `-32024` builder.
It always returns the selected instance in `known_ids`; `reason`, nullable structured
`cause`, `socket_path`, `offending_path`, `log_path`, and
`durable_state: preserved`; plus managed status, read-only path inspection, and
read-only log inspection actions. The `ensure_running()` boundary normalizes existing
socket lifecycle faults through that builder, while spawn and readiness branches call
it directly. Stop and raw/advanced behavior are unchanged.

Follow-up RED:

```text
5 representative lifecycle/public tests
FAILED (failures=5)
```

The failures demonstrated incomplete details for accepting socket, spawn, early exit,
timeout, and main/CLI unsafe-lock refusal.

Follow-up focused GREEN:

```text
python3 -W error::ResourceWarning -m unittest \
  tests/codex-worker/test_instance.py tests/codex-worker/test_rpc_cli.py -v

Ran 91 tests in 16.641s
OK
```

Follow-up warning-strict fast gate:

```text
python3 -W error::ResourceWarning -m unittest discover \
  -s tests/codex-worker -p 'test_*.py'

283 discovered tests; exit 0
```

Follow-up `py_compile` and `git diff --check` both exited 0.

No new error code, kind, CLI method, destructive action, or compatibility deviation
was introduced.
