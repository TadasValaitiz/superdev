...
const SUPERDEV_VERSION = readSuperdevVersion();
const SUPERDEV_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/superpowers-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'SUPERDEV_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SUPERDEV_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(SUPERDEV_VERSION);
  const text = SUPERDEV_TELEMETRY_DISABLED
    ? 'Prime Radiant Superdev v' + version
    : 'Superdev v' + version;
  const logo = SUPERDEV_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SUPERDEV_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(SUPERDEV_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';
