...
```sh
codex-worker start --name implement-a31 --prompt-file implement-a.md &
codex-worker start --name implement-b32 --prompt-file implement-b.md &
codex-worker start --name review-c33 --prompt-file review-c.md &
codex-worker start --name review-d34 --prompt-file review-d.md &
codex-worker start --name verify-e35 --prompt-file verify-e.md &
```

Use the harness's normal join/wait mechanism. Each worker needs its own appropriate
worktree and the usual SDD handoffs; no implementer is its own independent reviewer.
...
A `start`/`run` timeout is a local wait limit, not cancellation. Use the returned
`status`, `messages`, or `interrupt` recovery command by name. If durable persistence
...
thread selectors. Keep this escape hatch separate from ordinary named-worker work.
